...
int main()
{
    ...
    //以光的透视图进行场景渲染的时候，我们会用一个比较简单的着色器，这个着色器simpleDepthShader除了把顶点变换到光空间以外，不会做得更多了。
    //这个顶点着色器3.1.1.shadow_mapping_depth.vs将一个单独模型的一个顶点，使用lightSpaceMatrix变换到光空间中。
    ...
    glBindTexture(GL_TEXTURE_2D, depthMap);//创建一个2D纹理，提供给帧缓冲的深度缓冲使用
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, SHADOW_WIDTH, SHADOW_HEIGHT, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);//因为我们只关心深度值，我们要把纹理格式指定为GL_DEPTH_COMPONENT
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render scene from light's point of view渲染深度缓冲
        simpleDepthShader.use();
        simpleDepthShader.setMat4("lightSpaceMatrix", lightSpaceMatrix);

        glViewport(0, 0, SHADOW_WIDTH, SHADOW_HEIGHT);//视口匹配纹理的分辨率
        glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
        glClear(GL_DEPTH_BUFFER_BIT);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, woodTexture);
        renderScene(simpleDepthShader);//调用所有相关的绘制函数，并在需要的地方设置相应的模型矩阵。
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        // --------------------------------------------------------------
        glViewport(0, 0, SCR_WIDTH, SCR_HEIGHT);// reset viewport将视口重置回原来的大小（800*600）
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        shader.use();
        ...
        // 在光的透视图视角下，很完美地用每个可见片段的最近深度填充了深度缓冲。
        // 通过将这个纹理投射到一个2D四边形上（和我们在帧缓冲一节做的后处理过程类似），就能在屏幕上显示出来
