...
int main()
{
    ...
    // -------------------------
    //在这个教程中，我们将会在一个简化版本的延迟渲染器(延迟着色法教程中)的基础上实现SSAO
    ...
    //环境遮蔽模糊:由于平铺的随机向量纹理保持了一致的随机性，我们可以使用这一性质来创建一个简单的模糊着色器
    ...
    // -----------------------------------------------------
    // SSAO着色器在2D的铺屏四边形上运行，它对于每一个生成的片段计算遮蔽值(为了在最终的光照着色器中使用)。由于我们需要存储SSAO阶段的结果，我们还需要在创建一个帧缓冲对象ssaoFBO
    ...
    // and blur stage
    // 在SSAO阶段和光照阶段之间，我们想要进行模糊SSAO纹理的处理，所以我们又创建了一个帧缓冲对象ssaoBlurFBO来储存模糊结果。
    ...
    // 生成样本核
    // 我们需要沿着表面法线方向生成大量的样本。
    // 我们将在切线空间(Tangent Space)内生成采样核心，法向量将指向正z方向。
    // ----------------------
    ...
    for (unsigned int i = 0; i < 64; ++i)
    {
        ...
        //std::cout << randomFloats(generator) * 2.0 - 1.0<<endl;
        ...
        // 目前，所有的样本都是平均分布在采样核心里的，但是我们更愿意将更多的注意放在靠近真正片段的遮蔽上，也就是将核心样本靠近原点分布。
        // 我们可以用一个加速插值函数实现它:
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderGeometryPass.setInt("invertedNormals", 1); // invert normals as we're inside the cube法线反向因为在cube内部
        renderCube();
        shaderGeometryPass.setInt("invertedNormals", 0);
        // ourModel model on the floor
        model = glm::mat4(1.0f);
        ...
        shaderSSAOBlur.use();
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, ssaoColorBuffer);//输入SSAO纹理
        renderQuad();
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        glBindTexture(GL_TEXTURE_2D, ssaoColorBufferBlur);//输入经过模糊处理之后的ssaoColorBufferBlur纹理（一个包含逐片段环境遮蔽数据的纹理；在光照处理阶段中可以直接使用。）
        renderQuad();
